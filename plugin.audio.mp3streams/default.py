...
import six
from six.moves import range, urllib,urllib_parse, zip
...
ARTIST_FANART_ART = settings.artist_fanart()
...
fanart_tv_api_key = '9baad15ec12f719ca73853728ff17310'
fanart_tv_headers = {'api-key': fanart_tv_api_key}
fanart_tv_art_link = 'https://fanart.tv/api/search.php?section=Music&s='
fanart_artist_page = 'http://webservice.fanart.tv/v3/music/'
...
def search_artists(query):
    ...
    for url1, title in all_artists:
        icon_path = os.path.join(ARTIST_ART, title + '.jpg')
        fanart_path = os.path.join(ARTIST_FANART_ART, title + '.jpg')
        if os.path.exists(icon_path):
            iconimage = icon_path
        else:
            iconimage = iconart
        if os.path.exists(fanart_path):
            ...
            fanartimage = fanart
        addArtistDir(title.replace('&amp;', 'and'),'http://musicmp3.ru' + url1,22,iconimage,fanartimage,'artists')
...
def search_albums(query):
    ...
    for url1,thumb,album,plot,artisturl,artist,year in all_albums:
        ...
        fanart_path = os.path.join(ARTIST_FANART_ART, artist + '.jpg')
        if os.path.exists(fanart_path):
            fanartimage = fanart_path
        else:
            fanartimage = fanart
        addArtistDir(title.replace('&amp;', 'and'),'http://musicmp3.ru' + url1,5,thumb,fanartimage,'albums')
    setView('files', 'album')
...
def albums(name, url):
    ...
    for url1,d1,thumb,album,d2,year in all_albums:
        ...
        if title not in duplicate:
            ...
            thumb = thumb.replace('al', 'alm').replace('covers', 'mcovers')
            fanart_path = os.path.join(ARTIST_FANART_ART, name + '.jpg')
            if os.path.exists(fanart_path):
                fanartimage = fanart_path
            else:
                fanartimage = fanart
...
def play_album(name, url, iconimage, mix, clear):
    ...
    for track,id,songurl,meta, d1,album,artist,songname,dur in match:
        ...
        if os.path.exists(fanart_path):
            ...
            fanartimage = fanart
        if 'musicmp3' in origurl:
            url, liz = playerMP3.getListItem(songname, artist, album, trn, iconimage, dur, url, fanartimage, 'true', GOTHAM_FIX_2)
        elif 'goldenmp3' in origurl:
            url, liz = playerMP3.getListItem(ntrack, songname, album, trn, iconimage, dur, url, fanartimage, 'true', GOTHAM_FIX_2)
        else:
            url, liz = playerMP3.getListItem(songname, artist, album, trn, iconimage, dur, url, fanartimage, 'true', GOTHAM_FIX_2)
...
def play_song(url, name, songname, artist, album, iconimage, dur, clear):
    ...
    if os.path.exists(fanart_path):
        ...
        fanartimage = fanart
    url, liz = playerMP3.getListItem(songname, artist, album, track, iconimage, dur, url, fanartimage, 'true', GOTHAM_FIX_2)
    title=name
    if FOLDERSTRUCTURE=="0":
...
def get_artist_icon(name, url):
    ...
    fanartlink = fanart_tv_art_link + name
    if not os.path.exists(data_path):
        art = client.scrapePage(fanartlink,timeout='30').json()
        try:
            artistid = art[0].get('link')
            searchresult = re.search('\/artist\/(.*?)\/', artistid)
            result_artist_id = searchresult.group(1)
            artinfo = client.scrapePage(fanart_artist_page + result_artist_id + '?api_key=' + fanart_tv_api_key,timeout='30').json()
            artist_thump = artinfo.get('artistthumb')
            first_link = artist_thump[0].get('url')
            xbmc.log("728 artistthumblink = {0}".format(first_link), xbmc.LOGINFO)
            response = requests.get(first_link, headers=fanart_tv_headers, stream=True)
            with open(data_path,'wb') as fd:
                for chunk in response.iter_content(chunk_size=128):
                    fd.write(chunk)
            artist_fanart = artinfo.get('artistbackground')
            first_background_link = artist_fanart[0].get('url')
            background_response = requests.get(first_background_link, headers=fanart_tv_headers, stream=True)
            with open(fanart_path,'wb') as f:
                for chunk in background_response.iter_content(chunk_size=128):
                    f.write(chunk)
        except:
            xbmc.log("728 imageurl = {0}".format('failed'), xbmc.LOGINFO)
        ...
        for list in search_list:
            if list != '':
                ...
                if os.path.exists(fanart_path):
                    ...
                    fanartimage = fanart
                addArtistDir(title.replace('&amp;', '&').upper(),url,22,iconimage,fanartimage,'artists')
...
def addArtistDir(name, url, mode, iconimage, fanartimage, type):
        type1 = type
        ...
        liz.setArt({'icon': "DefaultAudio.png", 'thumb': iconimage, 'fanart' : fanartimage}) 
...
def addDirAudio(name, url, mode, iconimage, songname, artist, album, dur, type):
        ...
        liz.setArt({'icon': "DefaultAudio.png", 'thumb': iconimage, 'fanart' : fanartimage}) 
        liz.addContextMenuItems(contextMenuItems, replaceItems=False)
        liz.setInfo( type="Audio", infoLabels={ "Title": name, 'artist': artist, 'album': album, 'duration' : dur } )
